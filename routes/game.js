...
router.post('/addGame', async (req, res) => {
  const { name, size, genre, letter, added, pinyin, src, no, platform, rate } = req.body
  ...
  const ret = await addGame(name, size, genre, letter, added, pinyin, src, no, rate, platform, creatAt)
...
router.post('/addGames', async (req, res) => {
  ...
  if (Array.isArray(games)) {
    games.forEach(async (item) => {
      const { name, size, genre, letter, added, pinyin, src, no, rate, platform } = item
      const createAt = Date.now()
      await addGame(name, size, genre, letter, added, pinyin, src, no, rate, platform, createAt)
    })
